...
survey1_id=$(curl -sf -XPOST ${API_URL}/surveys -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"survey_title":"Spring Fest 2023", "start_date_time":"2023-03-17T00:00:00.000+09:00", "end_date_time":"2024-03-17T00:00:00.000+09:00"}' | jq -r .survey_id)

question_group1_id=$(curl -sf -XPOST ${API_URL}/question_groups -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_group_title":"イベント全体の感想", "question_group_type": "イベント全体の感想"}' | jq -r .question_group_id)
question_group2_id=$(curl -sf -XPOST ${API_URL}/question_groups -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_group_title":"セッションAの感想", "question_group_type": "セッション毎の感想"}' | jq -r .question_group_id)
question_group3_id=$(curl -sf -XPOST ${API_URL}/question_groups -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_group_title":"セッションBの感想", "question_group_type": "セッション毎の感想"}' | jq -r .question_group_id)
question_group4_id=$(curl -sf -XPOST ${API_URL}/question_groups -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_group_title":"セッションCの感想", "question_group_type": "セッション毎の感想"}' | jq -r .question_group_id)

question1_id=$(curl -sf -XPOST ${API_URL}/questions -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_text": "満足度はどうだったでしょうか？", "max_choices": 1}' | jq -r .question_id)
question2_id=$(curl -sf -XPOST ${API_URL}/questions -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_text": "次回、期待するコンテンツを教えてください"}' | jq -r .question_id)
question3_id=$(curl -sf -XPOST ${API_URL}/questions -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_text": "全体で何かご意見があれば教えてください"}' | jq -r .question_id)
question4_id=$(curl -sf -XPOST ${API_URL}/questions -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_text": "難易度は良かったでしょうか？", "max_choices": 1}' | jq -r .question_id)
question5_id=$(curl -sf -XPOST ${API_URL}/questions -u ${USERNAME}:${PASSWORD} -H 'Content-Type: application/json' -d '{"question_text": "何かご意見があれば教えてください"}' | jq -r .question_id)
...
curl -sf -XPOST ${API_URL}/surveys/${survey1_id}/answers -H 'Content-Type: application/json' -u ${USERNAME}:${PASSWORD} -d "{\"question_group_id\": \"${question_group1_id}\", \"question_id\": \"${question1_id}\", \"respondent_id\": \"demo3\", \"choices\": [{\"question_choice_id\": \"${question1_choice2_id}\"}]}"
curl -sf -XPOST ${API_URL}/surveys/${survey1_id}/answers -H 'Content-Type: application/json' -u ${USERNAME}:${PASSWORD} -d "{\"question_group_id\": \"${question_group1_id}\", \"question_id\": \"${question1_id}\", \"respondent_id\": \"demo4\", \"choices\": [{\"question_choice_id\": \"${question1_choice2_id}\"}]}"
curl -sf -XPOST ${API_URL}/surveys/${survey1_id}/answers -H 'Content-Type: application/json' -u ${USERNAME}:${PASSWORD} -d "{\"question_group_id\": \"${question_group1_id}\", \"question_id\": \"${question1_id}\", \"respondent_id\": \"demo5\", \"choices\": [{\"question_choice_id\": \"${question1_choice3_id}\"}]}"
curl -sf -XPOST ${API_URL}/surveys/${survey1_id}/answers -H 'Content-Type: application/json' -u ${USERNAME}:${PASSWORD} -d "{\"question_group_id\": \"${question_group1_id}\", \"question_id\": \"${question1_id}\", \"respondent_id\": \"demo6\", \"choices\": [{\"question_choice_id\": \"${question1_choice4_id}\"}]}"
